...
export const GamePlay = {
    ColorIntentHandler(handlerInput: HandlerInput) {
        ...
        const uColor = (request as IntentRequest).intent.slots!.color.value;
    ...
    HandleButtonPressed(handlerInput: HandlerInput) {
        console.log("GamePlay::InputHandlerEvent::button_down_event");
        const { attributesManager } = handlerInput;
        const ctx = attributesManager.getRequestAttributes();
        const sessionAttributes = attributesManager.getSessionAttributes();

        const deviceIds = sessionAttributes.DeviceIDs;
        ...
        if (deviceIds.indexOf(buttonId) == -1) {
            ...
            const buttonNo = deviceIds.indexOf(buttonId);
